...
<div class="col-md-6">
		...
		@if(session('success'))
			...
			<div class="contact-form">
				<h4 class="text-left"><i class="fas fa-envelope text-info"></i> &nbsp;Contacteer ons,<br>We horen graag van jou!</h4>
            <form action="{{route('contact-email')}}" method="post"> 
            @csrf
    
				<div class="form-group">
				  <label class="control-label" for="fname">Voor & Achternaam:</label>
					<input type="text" class="form-control" id="fname" name="firstname" placeholder="Voor & Achternaam" name="firstname">
					@if ($errors->has('firstname')) <p style="color:red;">{{ $errors->first('firstname') }}</p> @endif 
				</div>
				<div class="form-group">
				  <label class="control-label" for="lname">telefoonnummer:</label> 
					<input type="text" class="form-control" id="lname" name="lastname" placeholder="telefoonnummer" name="lastname">
